...
export function AdminSidebar({ employeeName, permissions = [] }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:sticky top-0 right-0 z-30 bg-[var(--color-forest-900)] text-white flex flex-col h-screen font-sans border-l-4 border-[var(--color-champagne-600)] transition-all duration-300 transform md:transform-none ${
      ...
      >
        ...
        {/* Brand Header */}
        <div className={`p-6 border-b border-[var(--color-forest-800)] flex flex-col items-center transition-all relative ${isCollapsed ? 'md:p-3 md:pb-4' : ''}`}>
          {/* Mobile Close Button */}
          <button 
            onClick={() => setIsOpen(false)}
            className="md:hidden absolute top-4 left-4 p-2 text-zinc-400 hover:text-white rounded-full hover:bg-[var(--color-forest-800)] transition-colors"
          >
        ...
        <div className={`p-4 border-b border-[var(--color-forest-800)] text-xs text-zinc-300 flex ${isCollapsed ? 'md:flex-col md:gap-2' : 'justify-between items-center'}`}>
          ...
          {filteredLinks.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                onClick={() => setIsOpen(false)}
                className={`flex items-center gap-3 px-4 py-2.5 rounded hover:bg-[var(--color-forest-800)] text-sm font-medium transition-colors text-zinc-200 hover:text-white ${isCollapsed ? 'md:justify-center md:px-2' : ''}`}
                title={link.label}
              >
