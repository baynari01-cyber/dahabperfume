      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 font-sans">
        ...
        <div className="max-w-4xl space-y-8">
          {pages.map((page) => (
            <form key={page.key} action={handleSavePageCMS} className="bg-white p-6 rounded-lg border border-[var(--color-ivory-200)] shadow-sm space-y-6">
              <div className="flex justify-between items-center border-b pb-2 mb-4">
                <h2 className="text-xl font-bold text-[var(--color-forest-900)]">{page.label}</h2>
                <button type="submit" className="bg-[var(--color-forest-900)] text-white px-6 py-2 rounded text-sm font-bold hover:bg-[var(--color-forest-800)] transition-colors">
                  حفظ ({page.label})
                </button>
              </div>
          ...
          <form action={handleSavePageCMS} className="bg-white p-6 rounded-lg border border-[var(--color-ivory-200)] shadow-sm space-y-6">
            <div className="flex justify-between items-center border-b pb-2 mb-4">
              <h2 className="text-xl font-bold text-[var(--color-forest-900)]">الشحن والتوصيل (مقدمة الصفحة)</h2>
              <button type="submit" className="bg-[var(--color-forest-900)] text-white px-6 py-2 rounded text-sm font-bold hover:bg-[var(--color-forest-800)] transition-colors">
                حفظ (الشحن)
              </button>
            </div>
